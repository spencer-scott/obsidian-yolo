...
- Quick Ask / Sidebar Chat / Agent Chat: all three share the same agent runtime (`AgentService.run` -> `loop-worker` + `AgentToolGateway`) and resolve `loopConfig` / `allowedToolNames` / `toolPreferences` through `resolveChatModeRuntime` (`src/components/chat-view/chat-runtime-profiles.ts`). Chat mode applies the `CHAT_BLOCKED_TOOL_NAMES` blocklist (blocking fs-write tools) and does not pass `toolPreferences`; Agent mode passes the full tool set and preferences. `maxAutoIterations` defaults to 100. Quick Ask and sidebar have no runtime-level differences, only UI form factor differs.
- Smart Space / Write Assist: low-latency editing scenarios that directly reuse `src/core/ai/single-turn.ts` without going through the agent runtime.
